        ...
        switch (followMode)
        {
            ...
            case FollowMode.CHASE:
                if (!Input.GetKey(freezeKey))
                {
                    ...
                    //_cacheTransform.rotation = Quaternion.Lerp(_cacheTransform.rotation, _lookAt, Time.deltaTime * lookAtDamping);
                    _cacheTransform.rotation = Quaternion.Lerp(_cacheTransform.rotation, _lookAt, 1);
                    // Smooth follow interpolation
                    _cacheTransform.position = Vector3.Lerp(_cacheTransform.position, target.position - target.forward * distance + target.up * chaseHeight, Time.deltaTime * chaseDistance);
                }
                break;
