...
public class SU_Spaceship : MonoBehaviour {
	...
	void Update ()
    {
        // Start all thrusters when pressing Fire 

        if (InputManager.GetButton("Thrust")) {		
			foreach (SU_Thruster _thruster in thrusters)
        ...
        if (InputManager.GetButton("Afterburner"))
        {
    ...
    void MotionControl()
    {
        if (InputManager.GetAxis("RudderRight") >= 0.01)
            RudderAxis = InputManager.GetAxis("RudderRight") / sensitivity;
        else RudderAxis = InputManager.GetAxis("RudderLeft") >= 0.01 ? InputManager.GetAxis("RudderLeft") / sensitivity*(-1) : 0;

        // In the physics update...
        // Add relative rotational roll torque when steering left/right
        _cacheRigidbody.AddRelativeTorque(new Vector3(0, 0, -InputManager.GetAxis("Horizontal")/ sensitivity * rollRate * _cacheRigidbody.mass));
        // Add rudder yaw torque when steering left/right
        _cacheRigidbody.AddRelativeTorque(new Vector3(0, RudderAxis * yawRate * _cacheRigidbody.mass, 0));
        // Add pitch torque when steering up/down
        _cacheRigidbody.AddRelativeTorque(new Vector3(InputManager.GetAxis("Vertical")/ sensitivity * pitchRate * _cacheRigidbody.mass, 0, 0));
    }
}
